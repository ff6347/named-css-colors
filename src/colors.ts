import { getHue, hexToRgb, rgbToHsl } from "./helpers";
...
export const colors = () => {
	...
	type ColorItem = (typeof c)[number];
	const hashmap: Record<string, ColorItem> = {};
	c.forEach((item) => {
		hashmap[item.name] = item;
	});
	return { items: c, hashmap };
};

/**
 * This was the original dataset used for the alfred workflow
 * Keeping it around to still be able to use the old structure
 */
const colorsRaw = {
	items: [
		{
